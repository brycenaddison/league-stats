...
class Match {
	...
	- access in v4
	*/
    constructor(MATCH_CHAMPIONS, m, json = false) {
        ...
        if (this.version === 4 && this.mValid) {
            ...
            for (let participant of this.m_raw.participants) {
                this.m.participants.push({
                    ...
                    profileIcon: this.m_raw.participantIdentities.find(
                    ...
                    summonerId: this.m_raw.participantIdentities.find(
                        (x) => x.participantId == participant.participantId
                    ).player.summonerId,
                    summonerLevel: null,
                    summonerName: this.m_raw.participantIdentities.find(
                        (x) => x.participantId == participant.participantId
                    ).player.summonerName,
            ...
            for (let participant of this.m_raw.info.participants) {
                try {
                    participant.championId = matchFindChampion(
        ...
        if (this.version === 4) {
            ...
            const patch =
                parseInt(
                    game_version.substring(0, game_version_first_dot_index)
                ) *
                    100 +
                parseInt(
                    game_version.substring(
                        game_version_first_dot_index + 1,
                        game_version.indexOf(
                            '.',
                            game_version_first_dot_index + 1
                        )
                    )
        ...
        this.cache.participantIdentities = this.m.participants.map(
            (participant) => {
                return {
                    participantId: participant.participantId,
                    player: {
