...
const match_id = document.URL.substring(document.URL.lastIndexOf('/')+1);

let match_url = 'http://api.brycenaddison.com/m/' + match_id;
...
loadJSON(match_url)
    .then((match_data) => {
        let match = new Match(champion_data, match_data, true);
        const major_patch = match.gameVersion.substring(
            0,
            match.gameVersion.indexOf('.', match.gameVersion.indexOf('.') + 1)
        );
    ...
    <p class="m-1">Region: ${escapeHtml(
        regions[match.platformId]
    )}, Match ID: ${escapeHtml(match.gameId)}, Patch ${escapeHtml(
            major_patch
        )}, Duration: ${standardTimestamp(match.gameDuration)}</p>`;
        Promise.all([
            loadJSON(
                `https://ddragon.leagueoflegends.com/cdn/${addv}/data/en_US/champion.json`
            ),
            loadJSON(
                `https://ddragon.leagueoflegends.com/cdn/${addv}/data/en_US/summoner.json`
            ),
            loadJSON(
                `https://ddragon.leagueoflegends.com/cdn/${addv}/data/en_US/runesReforged.json`
            )
        ])
            .then((responses) => {
                console.log(responses);
                champion_data = responses[0];
                spell_data = responses[1];
                rune_data = responses[2];
                match = new Match(
                    champion_data,
                    match_data,
                    true
            ...
            ${headerText('Spells')}
            ${headerText('Level')}
            <th>Champion ${team.bans
                .map(
                    (ban) =>
                        `<div style="border: 2px solid red; display: inline-block;">${championIDtoImg(
                            ban.championId,
                            'champion-ban-img'
                        )}</div>`
                )
            ...
            ${headerText('CS')}
            ${headerText('Gold')}
            </tr></thead>${match.participants
                .map((p) => {
                    if (p.teamId != team.teamId) return '';
                    let pI = match.participantIdentities.find(
                        (pI) => p.participantId == pI.participantId
                    );
                    return `<tr class="match-${
                        p.stats.win ? 'victory' : 'defeat'
                    }">
                ...
                ${cellText(
                    p.stats.neutralMinionsKilled + p.stats.totalMinionsKilled
                )}
                ...
                for (let participant_id in match.participants) {
                    for (let prop_name in match.participants[participant_id]
                        .stats) {
                        if (
                            !exclude_stat_name.includes(prop_name) &&
                            !participant_stat_props.includes(prop_name)
                        ) {
        ...
        <tr>${headerText('Summoner Name')}${match.participants
                    .map((p) => {
                        let pI = match.participantIdentities.find(
                            (pI) => p.participantId == pI.participantId
                        );
                    ...
                    .join('')}</tr>
        <tr>${headerText('Champion')}${match.participants
                    .map((p) => {
                        return `<th>${championIDtoImg(p.championId)}</th>`;
                    })
                    .join('')}</tr>
        </thead>
        ${participant_stat_props
            .map((prop_name) => {
                return `<tr>${headerText(prop_name, 'tal')}${match.participants
                    .map((p) => {
                        let classes = '';
                        if (p.stats[prop_name] === true) {
                            ...
                            p.stats[prop_name] === null ||
                            p.stats[prop_name] === undefined
                        ) {
                            return cellText('');
                        }
                        if (stat_value_override[prop_name]) {
                            return stat_value_override[prop_name](
                                p.stats[prop_name]
                            );
                        } else {
...
function championIDtoImg(id, img_class = 'champion-img') {
    for (let b in champion_data.data) {
        if (champion_data.data[b].key == id) {
            return `<img${
                img_class == '' ? '' : ` class=${escapeHtml(img_class)}`
            } src="https://ddragon.leagueoflegends.com/cdn/${encodeURIComponent(
                addv
            )}/img/champion/${encodeURIComponent(b)}.png">`;
        }
    }
...
function itemIDtoImg(id, img_class = 'item-img') {
    ...
    return `<img class="${img_class}" src="https://ddragon.leagueoflegends.com/cdn/${encodeURIComponent(
        addv
    )}/img/item/${encodeURIComponent(id)}.png">`;
}
...
function spellIDtoImg(id, img_class = 'spell-img') {
    for (let b in spell_data.data) {
        if (spell_data.data[b].key == id) {
            return `<img class="${img_class}" src="https://ddragon.leagueoflegends.com/cdn/${encodeURIComponent(
                addv
            )}/img/spell/${encodeURIComponent(spell_data.data[b].id)}.png">`;
        }
    }
    return '';
}
